      ...
      <v-card :loading="loading" dark>
        ...
        <v-card-text class="mt-4" v-if="!loading">
          ...
          <v-alert
            ...
            prominent
          >
            <strong>Cultura: </strong> {{ item.name }}
            <v-spacer></v-spacer>
            <strong>Umidade: </strong> {{ item.soil_moisture }} (cm)
            <v-spacer></v-spacer>
            <strong>Inserido em: </strong>{{ created_at_datetime()
            }}
          </v-alert>
        </v-card-text>
...
export default {
  ...
  methods: {
    ...
    deleteData() {
      this.loading = true;
      this.$emit('deleteData', this.item.id);
    },
    created_at_datetime() {
      let br_date = '';
      let br_time = '';
