    ...
    <v-card dark class="hover-card" elevation="7" outlined shaped>
      ...
      <v-card-subtitle>
        <span style="position: absolute; top: 60px" class="subtitle-card-dark"
          >Cadastre suas culturas de trabalho</span
        >
      </v-card-subtitle>
      <v-card-text>
        <v-data-table
...
<script>
import ModalAdd from './Modals/ModalAddCulture.vue';
import ModalEdit from './Modals/ModalEditCulture.vue';
import ModalDelete from './Modals/ModalDeleteCulture.vue';
export default {
  name: 'CulturesComponent',
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Umidade', value: 'soil_moisture', align: 'start' },
  ...
  methods: {
    addData(item) {
      this.$http.post('add_culture', item).then((response) => {
    ...
    editData(item) {
      this.$http.put(`edit_culture/${item.id}`, item).then((response) => {
